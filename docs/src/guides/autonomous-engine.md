...
// Check status
const status = await toolkit.executeTool('a2a_billing_executor_status', {});
// → { running: true, totalTicks: 142, chargesProcessed: 87, failedCharges: 3 }

// Manually trigger for a specific subscription
await toolkit.executeTool('a2a_billing_run', { subscriptionId: 'sub-123' });
```
...
const status = await toolkit.executeTool('a2a_dispute_resolver_status', {});
...
const compliance = await toolkit.executeTool('a2a_sla_check', {
    agentId: 'data-agent',
    slaId: 'uptime-99'
});

// Auto-enforce penalties
await toolkit.executeTool('a2a_sla_enforce_penalties', {
    slaId: 'uptime-99',
    period: '2026-03'
});
```
...
| Tool | Description |
|------|-------------|
| `a2a_billing_executor_status` | Billing executor health and metrics |
| `a2a_billing_run` | Trigger billing cycle for a subscription |
| `a2a_dispute_resolver_status` | Dispute resolver health and metrics |
| `a2a_sla_check` | Check SLA compliance for an agent |
| `a2a_sla_enforce_penalties` | Apply SLA penalties for a period |
| `a2a_marketplace_auto_award` | Auto-award an RFQ based on scoring |
| `a2a_health_status` | Full health check (DB + sequencer + subsystems) |
| `a2a_health_live` | Liveness probe |
| `a2a_health_ready` | Readiness probe (DB connectivity) |
