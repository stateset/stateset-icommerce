...
The benchmark crate includes checked-in perf gates for arithmetic, validation, Merkle construction, event throughput, and SQLite batch insert workloads. CI can enforce those budgets through `STATESET_PERF_GATE=1`, which reads thresholds from `crates/stateset-benches/perf-gates.json`.
