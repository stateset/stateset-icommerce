...
StateSet iCommerce ships a broad set of language bindings across Rust, Node.js, Python, Ruby, PHP, Java, Kotlin, Swift, C#/.NET, Go, and WASM. Hand-maintaining each binding would create drift, inconsistent behavior, and high maintenance cost as the API surface evolves.

We considered three approaches:

1. **Hand-written bindings** — Maximum flexibility per language, but maintenance cost scales as O(languages × API changes). Already impractical at the current binding count.
...
- New domain APIs (e.g., `gift_cards()`) automatically appear across the supported bindings
