...
const result = await tracing.withSpan('process_order', async () => {
    ...
    await toolkit.executeTool('create_payment', { orderId: order.id, amount: 59.98 });
    return order;
});
...
await toolkit.executeTool('audit_export', { format: 'json', since: '2026-03-01T00:00:00Z' });
```
...
```javascript
const dashboard = await toolkit.executeTool('a2a_agent_dashboard', {
    agentAddress: '0xfulfillment-agent'
});
...
| Compliance export | `audit_export` or `export_audit_trail` |
