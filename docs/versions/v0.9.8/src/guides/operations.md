...
5. `send_purchase_order` — deliver the approved PO to the supplier
6. When goods are received, `adjust_inventory` — add stock with the PO reference
...
5. `create_refund` — full or partial refund
...
2. `create_subscription` — create subscription with optional trial
...
Step 2: create_payment     ❌ failed
         ↓ compensation
Step 1: release_reservation  ✅ rolled back
```
...
```javascript
const failed = await toolkit.executeTool('a2a_list_webhook_dlq', {});
