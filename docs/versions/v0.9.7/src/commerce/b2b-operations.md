...
```javascript
// Partial receiving
await toolkit.executeTool('receive_purchase_order', {
    purchaseOrderId: 'po-001',
    receivedItems: JSON.stringify([
        { sku: 'COMPONENT-X', quantityReceived: 300, condition: 'good' },
        { sku: 'COMPONENT-Y', quantityReceived: 1000, condition: 'good' },
    ]),
    notes: 'Partial shipment — remaining 200 COMPONENT-X due March 28',
});
```

Receiving automatically adjusts inventory when configured:

```
Receive PO → adjust_inventory (quantity + 300, reason: "PO po-001 received")
```

### Closing a Purchase Order

```javascript
await toolkit.executeTool('close_purchase_order', {
    purchaseOrderId: 'po-001',
});
```
...
```javascript
await toolkit.executeTool('void_invoice', {
    invoiceId: 'inv-002',
    reason: 'Duplicate invoice — correct version is INV-003',
});
```
...
4. Approve PO               → approve_purchase_order (policy-gated)
5. Receive goods            → receive_purchase_order (partial or full)
6. Adjust inventory         → automatic stock increase
7. Receive supplier invoice → create_invoice (from supplier)
...
| `receive_purchase_order` | Record goods received |
| `close_purchase_order` | Close a completed PO |
