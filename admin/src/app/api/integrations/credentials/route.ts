...
import { validateBody, validateQuery, integrationCredentialSchema, paginationQuerySchema, safeIdSchema } from '@/lib/shared/schemas';
...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  if (!response.ok) {
    ...
    throw new AppError(errorData.error || 'Failed to fetch credentials', response.status, 'INTEGRATION_ERROR');
  }
...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  if (!response.ok) {
    ...
    const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
    throw new AppError(errorData.error || 'Failed to create credential', response.status, 'INTEGRATION_ERROR');
  }
...
export const DELETE = withErrorHandler(async (request: NextRequest) => {
  ...
  if (!response.ok) {
    ...
    const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
    throw new AppError(errorData.error || 'Failed to delete credential', response.status, 'INTEGRATION_ERROR');
  }
