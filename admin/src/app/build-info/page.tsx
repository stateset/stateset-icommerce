...
async function fetchVersion(): Promise<VersionResponse | { error: string }> {
  const apiUrl = getServerStateSetApiUrl();
  try {
    const response = await fetch(`${apiUrl}/version`, {
      // Build info changes only on deploy, but we want the page to
      // reflect the live running binary, so revalidate every 60s.
      next: { revalidate: 60 },
...
export default async function BuildInfoPage() {
  ...
  return <BuildInfoView result={result} />;
}

/**
 * Pure renderer split out from the async page so component tests can
 * exercise it directly without mocking `fetch` or the Next runtime.
 */
