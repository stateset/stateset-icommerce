  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <MetricCard label="Return Rate" value={formatPercentage(analytics.returnRate)} tone="warning" />
        <MetricCard label="Refund Total" value={formatCurrency(analytics.refundTotal)} tone="danger" />
        <MetricCard label="Avg Processing" value={`${analytics.averageProcessingTime}h`} tone="success" />
      </div>
              ...
              >
                <div className="text-center min-w-[100px]">
                  <div className={`mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-2 ${
                    ...
                    <span className={`font-bold ${stage.count > 0 ? 'text-ds-primary' : 'text-ds-muted-foreground'}`}>
                      {stage.count}
                    </span>
                  </div>
              ...
              {Object.entries(analytics.returnsByStatus || {}).map(([status, count]) => {
                ...
                return (
                  <div key={status}>
                    ...
                    <ProgressBar
                      value={percentage}
                      color={status === 'rejected' ? 'rose' : status === 'refunded' ? 'emerald' : 'indigo'}
                    />
                  </div>
                );
              })}
          ...
          <CardContent>
            <div className="space-y-3">
              {analytics.topReturnedProducts.slice(0, 5).map((product: { productId: string; name: string; count: number; rate: number }, index: number) => (
                <div key={product.productId} className="flex items-center justify-between p-3 border border-ds-enterprise-line rounded-lg">
              ...
              <TableBody>
                {returns.slice(0, 10).map((ret: Return) => (
                  <TableRow key={ret.id}>
                    ...
                    <TableCell className="text-sm font-mono">{ret.orderId.slice(0, 8)}...</TableCell>
                    <TableCell>
