...
function analyzeBody(body) {
  ...
  for (const tm of toolMatches) {
    ...
    if (
      name.includes('_') &&
      /^(list|get|create|update|delete|set|adjust|reserve|confirm|release|approve|reject|send|record|calculate|convert|format|enable|validate|apply|complete|cancel|abandon|pause|resume|skip|activate|deactivate|archive|deliver|ship|start|add|remove|vector|sync)/.test(
        name,
      )
    ) {
      mcpTools.add(name);
