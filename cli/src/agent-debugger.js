      ...
      examples.push({
        ...
        example: {
          ...
          step2a: {
            ...
            then: { tool: 'create_order', params: { customerId, items, currency } }
          },
          step2b: {
            if: 'stock.totalAvailable < orderQuantity',
            options: [
              { tool: 'adjust_inventory', params: { sku: 'PRODUCT-001', quantity: 100, reason: 'Restock' } },
              { tool: 'create_backorder', params: { orderId, sku: 'PRODUCT-001', quantity: orderQuantity - stock.totalAvailable } }
            ]
          }
        }
      });
    ...
    const recoveryStrategies = {
      insufficient_stock: async () => {
        ...
        return {
          ...
          reason: 'Insufficient stock requires manual intervention or inventory adjustment. Ask user how they want to proceed.'
        };
      },
      ...
      order_not_found: async () => {
        return {
          ...
          suggestion: 'Use list_orders to find the correct order ID, or create a new order with create_order'
        };
      }
    };

    const strategy = recoveryStrategies[analysis.pattern.name];
    return strategy ? strategy() : { canAutoRecover: false, reason: 'No auto-recovery strategy available' };
  }
