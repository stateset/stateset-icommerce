...
const INTENT_PATTERNS = {
  ...
  'cancel_order': [
    ...
    /void\s*(the\s*)?order/i,
    /delete\s*(the\s*)?order/i
  ],
  ...
  'approve_return': [
    /approve\s*(the\s*)?return/i,
    /accept\s*(the\s*)?return/i
  ],
  'reject_return': [
...
export class SuggestionEngine {
  ...
  getFuzzySuggestion(query) {
    ...
    if (corrections.length > 0) {
      const correctedQuery = words.map(w => {
  ...
  getContextualHelp(context = {}) {
    const { lastCommand, lastResult, error } = context;
    const suggestions = [];
