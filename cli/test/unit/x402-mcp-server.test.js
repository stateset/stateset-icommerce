...
describe(
  'createX402McpServer — server structure',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'createX402McpServer — tool properties',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let server;
    let tools;
...
describe(
  'x402_budget_status — default budget',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'x402_history — default budget (empty)',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'x402_budget_status — with budget env vars',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  ...
  () => {
    ...
    it('returns suggestion message when chain not provided', async () => {
      ...
      assert.ok(
        data.message.includes('budget_status') || data.message.includes('Chain not provided'),
      );
    });
  },
);
...
describe(
  'createX402McpServer — config from env',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'createX402McpServer — config from file',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'x402_call — ensureConfig validation',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'resolveSigningKey — path traversal prevention',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'loadKeyFromJson — via signing key env',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'loadKeyFromJson — via key file path',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'resolveKeyOnce — caching behavior',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'parseBool behavior — via X402_REQUIRE_RECEIPT',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'parseNumber behavior — via numeric env vars',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'parseList behavior — via X402_PREFERRED_NETWORKS',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
...
describe(
  'x402_history — with budget configured',
  { skip: !canImport && `import failed: ${importError?.message}` },
  () => {
    let tmpDir;
