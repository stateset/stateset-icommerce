...
itSerial('keeps interactive sessions idle-safe between turns and persists turn accounting', async () => {
  ...
  await waitFor(
    () => received.some((message) => message.type === 'result' && message.result === 'First turn complete'),
  );
...
itSerial('injects seeded conversation history into a fresh interactive Claude session', async () => {
  ...
  assert.ok(received.some((message) => message.type === 'result' && message.result === 'History-aware response complete'));
  ...
  assert.ok(events.some((event) => event.type === 'prompt_report' && event.report.historySource === 'conversation_history'));
...
itSerial('records treasury billing for persistent interactive Claude turns', async () => {
  ...
  assert.ok(received.some((message) => message.type === 'result' && message.result === 'Treasury turn complete'));
