...
_stateset_complete() {
    ...
    case "\${words[0]}" in
        ...
        stateset-mcp-events)
            if [[ "\${cur}" == -* ]]; then
                COMPREPLY=( $(compgen -W "--db --host --port --history-limit --stream-name --help --version" -- "\${cur}") )
            fi
            ;;
        stateset-channels)
            if [[ "\${cur}" == -* ]]; then
...
complete -F _stateset_complete stateset-create
    ...
    complete -F _stateset_complete stateset-mcp-events
...
_stateset_mcp_events() {
    _arguments -C \\
        '--db[Database path]:path:_files' \\
        '--host[Bind host]:host:' \\
        '--port[HTTP port]:port:' \\
        '--history-limit[History entries to keep]:count:' \\
        '--stream-name[Stream name]:name:' \\
        '--help[Show help]' \\
        '--version[Show version]' \\
        '*::arg:->args'
}
...
compdef _stateset_mcp_events stateset-mcp-events
...
complete -c stateset-mcp-events -f
...
# stateset-mcp-events
complete -c stateset-mcp-events -l db -d 'Database path' -r
complete -c stateset-mcp-events -l host -d 'HTTP host' -r
complete -c stateset-mcp-events -l port -d 'HTTP port' -r
complete -c stateset-mcp-events -l history-limit -d 'History limit' -r
complete -c stateset-mcp-events -l stream-name -d 'Stream name' -r
complete -c stateset-mcp-events -l help -d 'Show help'
complete -c stateset-mcp-events -l version -d 'Show version'
