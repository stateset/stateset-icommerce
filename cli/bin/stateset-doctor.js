 ...
 *   stateset-doctor --fix        Attempt to fix issues automatically
 */
...
async function autoFix(checkName, result) {
  if (!result.fixable) return false;

  switch (result.fixAction) {
    case 'create_dir': {
      try {
        fs.mkdirSync(result.fixTarget, { recursive: true });
        console.log(theme.success(`  Fixed: Created directory ${result.fixTarget}`));
        return true;
      } catch (err) {
        console.error(theme.error(`  Could not create directory: ${err.message}`));
        return false;
      }
    }
    case 'create_config_dir': {
      try {
        const configDir = path.join(os.homedir(), '.stateset');
        fs.mkdirSync(configDir, { recursive: true });
        fs.mkdirSync(path.join(configDir, 'profiles'), { recursive: true });
        console.log(theme.success('  Fixed: Created ~/.stateset/ directory'));
        return true;
      } catch (err) {
        console.error(theme.error(`  Could not create config dir: ${err.message}`));
        return false;
      }
    }
    case 'set_api_key': {
      try {
        ...
        const key = await ui.password('Enter your Anthropic API key');
        if (key) {
          const configDir = path.join(os.homedir(), '.stateset');
          if (!fs.existsSync(configDir)) {
            fs.mkdirSync(configDir, { recursive: true });
          ...
          const lines = ['# Generated by stateset-doctor --fix', `ANTHROPIC_API_KEY="${key}"`];
          fs.writeFileSync(envFile, lines.join('\n') + '\n', { mode: 0o600 });
          console.log(theme.success('  Fixed: API key saved to ~/.stateset/.env'));
          return true;
        }
      } catch {
        // @clack not available or user cancelled
      }
      return false;
    }
    case 'npm_install': {
      console.log(theme.muted('  To fix: run `npm install` from the project root'));
      return false;
    }
    default:
      return false;
...
async function main() {
  ...
  const _output = new RichOutput({ color: !wantsJsonOutput });
  ...
  // Try to use @clack tasks for animated output
  let ui = null;
  if (!wantsJsonOutput && process.stderr.isTTY) {
    try {
  ...
  if (!wantsJsonOutput) {
    if (ui) {
      ui.intro(`StateSet iCommerce Health Check  v${CLI_VERSION}`);
    } else {
      console.log(`\n${ICONS.analytics} ${theme.heading('StateSet iCommerce Health Check')}`);
      console.log(`${theme.muted(`   CLI v${CLI_VERSION}`)}\n`);
    }
  }
  ...
  for (const [name, result] of Object.entries(checks)) {
    const ms = timings[name] ? theme.muted(` (${timings[name]}ms)`) : '';
    let icon;
    ...
    console.log(`  ${icon} ${theme.bold(name)}: ${result.message}${ms}`);
    ...
    if (values.verbose && result.stats) {
      for (const [key, value] of Object.entries(result.stats)) {
        ...
        console.log(`     ${theme.muted(key + ':')} ${formatted}`);
      }
    }

    // Auto-fix if requested
    if (values.fix && result.fixable && result.status === 'error') {
      await autoFix(name, result);
    }
  }
  ...
  console.log();
  if (hasErrors) {
    const msg = 'Some checks failed. Please fix the errors above.';
    if (ui) {
      ui.logError(msg);
      if (!values.fix) {
        console.log(theme.muted('  Tip: Run with --fix to auto-repair what we can.\n'));
      }
    } else {
    ...
    const msg = 'All critical checks passed, but there are warnings.';
    if (ui) {
      ui.logWarning(msg);
    } else {
    ...
    if (ui) {
      ui.outro('All checks passed. System is healthy!');
    } else {
      console.log(theme.success('✓ All checks passed. System is healthy!'));
    }
    process.exit(0);
  }
}
