-- Give EVERY live customer a resolvable `email_key` (PostgreSQL twin of
-- SQLite migration 091; legacy-safe follow-up to 092).
--
-- Migration 092 keyed only addresses that were unambiguous at the time: rows
...
--
-- The expression index makes the grouping (and 092's correlated backfill on
...
UPDATE customers
SET email_key = LOWER(TRIM(email)) || ' ' || id::text
