...
impl PgCostAccountingRepository {
    ...
    pub async fn set_item_cost_async(&self, input: SetItemCost) -> Result<ItemCost> {
        ...
        self.get_item_cost_async(&input.sku)
            .await?
    ...
    pub async fn get_inventory_valuation_async(
        ...
        let (total_qty, total_val): (Decimal, Decimal) = sqlx::query_as(&sql)
            .fetch_one(&self.pool)
            .await
            .map_err(map_db_error)?;

        let average_unit_cost = if total_qty > Decimal::ZERO {
            total_val / total_qty
        } else {
            Decimal::ZERO
        };
