...
use crate::kernel::plans::orders::{
    CART_UNITS_SQL_POSTGRES, OrderTransitionSnapshot, ShipOrderSnapshot, plan_order_transition,
...
impl PgKernelExecutor {
    ...
    async fn execute_inventory_lifecycle_async<C: Serialize>(
        ...
        // confirms in full), so the figure the domain acts on is the minimum
        // of the two — binding to the raw payload amount would let a
        // commitment declare 1000 for a movement of 10.
        if let InventoryLifecycleAction::Confirm(quantity) = action
            && let Some(rejection) = economic_quantity_guard(
                command.commitment.as_ref(),
                quantity
                    .map_or(reservation.quantity, |declared| declared.min(reservation.quantity)),
            )
        {
            let mut receipt = run.rejected_by(&rejection);
    ...
    pub async fn execute_commit_checkout_async(
        ...
        if command.mode == ExecutionMode::Preview {
            ...
            // The SUM is the observation, so only pay for it when a quantity
            // was actually declared: without one the guard returns `None`
            // regardless.
...
async fn cart_units_pg(
    ...
    let units: i64 = sqlx::query_scalar(CART_UNITS_SQL_POSTGRES)
        .bind(cart_id)
        .fetch_one(tx)
        .await
        .map_err(pg_err)?;
