...
CREATE TABLE IF NOT EXISTS x402_credit_transactions (
    ...
    metadata TEXT,
    created_at TEXT NOT NULL,
    FOREIGN KEY (account_id) REFERENCES x402_credit_accounts(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_x402_credit_accounts_payer ON x402_credit_accounts(payer_address);
CREATE INDEX IF NOT EXISTS idx_x402_credit_accounts_asset_network ON x402_credit_accounts(asset, network);
CREATE INDEX IF NOT EXISTS idx_x402_credit_tx_account ON x402_credit_transactions(account_id);
CREATE INDEX IF NOT EXISTS idx_x402_credit_tx_payer ON x402_credit_transactions(payer_address);
CREATE INDEX IF NOT EXISTS idx_x402_credit_tx_direction ON x402_credit_transactions(direction);
CREATE INDEX IF NOT EXISTS idx_x402_credit_tx_reference ON x402_credit_transactions(reference_id);
