...
use axum::{Json, Router, extract::{Path, Query, State}, http::{HeaderMap, StatusCode}, routing::{delete, get, post}};
...
pub(crate) struct ShippingZoneResponse {
    pub id: String, pub name: String, pub countries: Vec<String>,
    pub is_active: bool, pub priority: i32, pub created_at: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, ToSchema)]
pub(crate) struct ShippingZoneListResponse { pub zones: Vec<ShippingZoneResponse>, pub total: usize }

fn zone_to_resp(z: &stateset_core::ShippingZone) -> ShippingZoneResponse {
    ShippingZoneResponse {
        id: z.id.to_string(), name: z.name.clone(),
        countries: z.countries.clone(), is_active: z.is_active,
        priority: z.priority, created_at: z.created_at.to_rfc3339(),
    }
}
...
pub(crate) async fn create_zone(
    State(state): State<AppState>, headers: HeaderMap, Json(req): Json<CreateShippingZoneRequest>,
...
pub(crate) async fn list_zones(
    State(state): State<AppState>, headers: HeaderMap, Query(params): Query<ShippingZoneFilterParams>,
...
pub(crate) async fn get_zone(
    State(state): State<AppState>, headers: HeaderMap, Path(id): Path<ShippingZoneId>,
) -> Result<Json<ShippingZoneResponse>, HttpError> {
    let tid = tenant_id_from_headers(&headers);
    let c = state.commerce_for_tenant(tid.as_deref())?;
    let z = c.shipping_zones().get(id)?.ok_or_else(|| HttpError::NotFound(format!("Zone {id} not found")))?;
