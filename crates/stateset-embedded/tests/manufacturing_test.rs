...
use stateset_embedded::{
    AddWorkOrderMaterial, BomStatus, Commerce, CreateBom, CreateBomComponent, CreateWorkOrder,
    CreateWorkOrderTask, TaskStatus, WorkOrderStatus,
};
use uuid::Uuid;
...
fn test_work_order_lifecycle() {
    ...
    let completed_task = commerce
        .work_orders()
        .complete_task(task.id, Some(dec!(2.5)))
    ...
    let consumed = commerce
        .work_orders()
        .consume_material(material.id, dec!(25))
        .expect("Failed to consume material");
