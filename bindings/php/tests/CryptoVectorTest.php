...
class CryptoVectorTest extends TestCase
{
    ...
    // Decode with assoc=false (objects as stdClass) so an empty JSON object `{}`
    // round-trips as `{}` rather than collapsing to a PHP `[]` — re-encoding it
    // with assoc arrays would canonicalize the wrong shape and break the digests.
    private function loadCorpus(): \stdClass
    {
