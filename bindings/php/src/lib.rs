...
impl BomApi {
    ...
    pub fn add_component(
        ...
        let cost = Decimal::from_f64_retain(unit_cost).unwrap_or_default();
...
impl CurrencyOps {
    ...
    pub fn set_rate(&self, from: String, to: String, rate: f64) -> PhpResult<ExchangeRate> {
        let commerce = lock_commerce!(self.commerce);
        let decimal_rate = Decimal::from_f64_retain(rate).unwrap_or_default();
...
impl Subscriptions {
    pub fn create_plan(
        ...
        let commerce = lock_commerce!(self.commerce);
        let decimal_price = Decimal::from_f64_retain(price).unwrap_or_default();
...
impl Promotions {
    pub fn create(
        ...
        let commerce = lock_commerce!(self.commerce);
        let decimal_value = Decimal::from_f64_retain(discount_value).unwrap_or_default();
        let decimal_min = min_purchase.map(|m| Decimal::from_f64_retain(m).unwrap_or_default());
    ...
    pub fn is_valid(&self, code: String, order_total: Option<f64>) -> PhpResult<bool> {
        let commerce = lock_commerce!(self.commerce);
        let decimal_total = order_total.map(|t| Decimal::from_f64_retain(t).unwrap_or_default());
