...
// =============================================================================
//
// Thin C-FFI wrappers over the `stateset-crypto` Rust crate so the Go binding
// can verify the language-neutral test corpus at
// `bindings/test-vectors/v1.json`. Counterparts in Rust:
// `crates/stateset-crypto/tests/cross_binding_vectors.rs`. Counterparts in
// Node and Python: see their respective `cross-binding-vectors` test files.
//
// All three return non-zero on error and zero on success; outputs use
// caller-allocated buffers where possible. The single variable-length output
// (`stateset_crypto_jcs_canonicalize`) returns a heap-allocated buffer that
// must be freed via `stateset_crypto_free_buffer`.
...
/// JCS-canonicalize a JSON string and return the canonical bytes.
///
/// Writes a heap-allocated `*mut u8` into `*out_ptr` and the length into
/// `*out_len`. Caller must free via `stateset_crypto_free_buffer(ptr, len)`.
///
...
/// `out_ptr` and `out_len` must be valid pointers writable for one element.
#[unsafe(no_mangle)]
...
/// Writes 32 bytes into `out_buf32`. If `salt_in` is non-null, `salt_len` must be 16.
///
...
/// if non-null it must point to `salt_len` bytes. `out_buf32` must point to a writable
/// 32-byte buffer.
#[unsafe(no_mangle)]
...
///
/// `leaves_in` is a contiguous buffer of `leaf_count * 32` bytes. Writes 32 bytes
/// into `out_buf32`. If `leaf_count == 0`, writes the empty-tree sentinel.
///
...
/// `leaf_count == 0`). `out_buf32` must point to a writable 32-byte buffer.
#[unsafe(no_mangle)]
