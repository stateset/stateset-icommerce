...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn stateset_string_free(s: *mut c_char) {
    if !s.is_null() {
...
#[unsafe(no_mangle)]
pub const extern "C" fn stateset_get_last_error() -> *mut c_char {
    // For now, return null - could implement thread-local error storage
    ptr::null_mut()
}
...
/// Create a customer
/// Returns JSON string (caller must free with `stateset_string_free`)
#[unsafe(no_mangle)]
...
pub extern "C" fn stateset_customer_count(handle: *mut CommerceHandle) -> c_int {
    ...
    match result {
        Ok(customers) => customers.len() as c_int,
        Err(_) => -1,
    }
}
...
pub extern "C" fn stateset_warehouse_create_location(
    ...
    let wh_id = match cstr_to_string(warehouse_id).and_then(|s| s.parse().ok()) {
        Some(id) => id,
        None => return ptr::null_mut(),
    };
...
pub extern "C" fn stateset_quality_create_inspection(
    ...
    let ref_type = cstr_to_string(reference_type).unwrap_or_else(|| "purchase_order".to_string());
    let ref_id = match cstr_to_string(reference_id).and_then(|s| s.parse().ok()) {
        Some(id) => id,
        None => return ptr::null_mut(),
    };
...
pub extern "C" fn stateset_ap_create_bill(
    ...
    let sup_id = match cstr_to_string(supplier_id).and_then(|s| s.parse().ok()) {
        Some(id) => id,
        None => return ptr::null_mut(),
    };
...
/// Returns JSON string (caller must free)
#[unsafe(no_mangle)]
pub extern "C" fn stateset_ar_customer_aging(
    handle: *mut CommerceHandle,
    customer_id: *const c_char,
) -> *mut c_char {
    let cust_id = match cstr_to_string(customer_id).and_then(|s| s.parse().ok()) {
        Some(id) => id,
        None => return ptr::null_mut(),
    };
...
pub extern "C" fn stateset_backorder_create(
    ...
    let ord_id = match cstr_to_string(order_id).and_then(|s| s.parse().ok()) {
        Some(id) => id,
        None => return ptr::null_mut(),
    };
    let cust_id = match cstr_to_string(customer_id).and_then(|s| s.parse().ok()) {
...
pub extern "C" fn stateset_gl_create_account(
    ...
    let acct_type = match cstr_to_string(account_type).as_deref() {
...
// =============================================================================
//
// Thin C-FFI wrappers over the `stateset-crypto` Rust crate so the .NET
...
//   - Go:     bindings/go/stateset/crypto_test.go (same FFI shape)
...
//   - Kotlin: bindings/kotlin/.../CryptoVectorTest.kt
//
// All return `0` on success and negative codes on errors. Variable-length
// outputs (canonical bytes) heap-allocate; caller frees via
...
/// # Safety
///
/// `ptr` must be a pointer returned by `stateset_crypto_jcs_canonicalize`,
/// `len` must match the length the caller received.
#[unsafe(no_mangle)]
...
///
/// Writes a heap-allocated `*mut u8` into `*out_ptr` and the length into
/// `*out_len`. Caller frees via `stateset_crypto_free_buffer(ptr, len)`.
///
...
/// must be 16.
///
/// Returns 0 on success; -1 on null/invalid input; -2 on hash error.
///
/// # Safety
///
/// `json_in` must be a valid NUL-terminated UTF-8 C string. `salt_in` may be
/// null; if non-null it must point to `salt_len` bytes. `out_buf32` must
/// point to a writable 32-byte buffer.
#[unsafe(no_mangle)]
...
/// sentinel from `stateset-crypto`.
///
/// Returns 0 on success; -1 on null/invalid input.
///
/// # Safety
///
/// `leaves_in` must point to at least `leaf_count * 32` bytes (or be null
/// when `leaf_count == 0`). `out_buf32` must point to a writable 32-byte
/// buffer.
#[unsafe(no_mangle)]
