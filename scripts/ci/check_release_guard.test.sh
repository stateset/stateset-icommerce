...
reset_state() {
  ...
  export GITHUB_REF="refs/tags/cli-v1.35.2"
...
assert_output_contains "Release guard passed for 1.35.2" "the guard must report the resolved version"

echo "==> derives the version from any of the three tag refs"
reset_state
GITHUB_REF="refs/tags/py-v1.35.2"
run_guard
...
FAKE_IS_ANCESTOR=0
run_guard
assert_status 1 "publishing from a commit that never landed must fail (the v1.35.2 failure)"
...
assert_output_contains "tag cli-v1.35.2 does not exist" "the missing cli tag must be named"
assert_output_contains "tag py-v1.35.2 does not exist" "the missing py tag must be named"

echo "==> rejects sibling tags that point at different commits"
reset_state
printf 'py-v1.35.2 %s\n' "$OTHER_SHA" >>"$FAKE_TAGS_FILE"
sed -i '/^py-v1.35.2 1111/d' "$FAKE_TAGS_FILE"
run_guard
...
run_guard
assert_status 1 "a red required check must block the publish (the v1.35.2 failure)"
