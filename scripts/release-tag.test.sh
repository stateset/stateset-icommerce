...
reset_state
FAKE_DIRTY=1
run_release_tag "1.35.3" --dry-run
...
run_release_tag "1.35.3" --dry-run
assert_status 1 "tagging off the protected branch must abort (the v1.35.3 failure)"
...
run_release_tag "1.35.3" --dry-run
assert_status 1 "a red required check must abort the release (the v1.35.3 failure)"
...
reset_state
printf 'Formatting\tcompleted\tfailure\t2026-09-08T11:00:00Z\n' >>"$FAKE_CHECK_RUNS_FILE"
run_release_tag "1.35.3" --dry-run --allow-red "Formatting"
...
reset_state
run_release_tag "1.35.3"
assert_status 0 "the happy path must succeed"
assert_log_contains "git tag -a v1.35.3 -m" "the v tag must be annotated"
assert_log_contains "git tag -a cli-v1.35.3 -m" "the cli-v tag must be annotated"
assert_log_contains "git tag -a py-v1.35.3 -m" "the py-v tag must be annotated"
assert_log_contains "git push --atomic origin refs/tags/v1.35.3 refs/tags/cli-v1.35.3 refs/tags/py-v1.35.3" \
...
assert_log_missing "git tag -a v1.35.3" "the existing tag must not be recreated"
assert_log_contains "git tag -a cli-v1.35.3 -m" "the missing sibling tags must still be created"
assert_log_contains "git push --atomic origin refs/tags/v1.35.3 refs/tags/cli-v1.35.3 refs/tags/py-v1.35.3" \
